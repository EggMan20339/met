...
(async () => {
  ...
  console.log(JSON.stringify({ before, hits, afterKill, hurt, preHeal, healed, errors }));
  await browser.close(); server.close();
  const ok = before.alive && !afterKill.alive && afterKill.soul > before.soul && hurt.hp < afterKill.hp && healed.hp === preHeal.hp + 1 && healed.soul === preHeal.soul - 33 && errors.length === 0;
